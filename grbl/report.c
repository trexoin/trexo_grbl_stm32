...
void report_status_message(uint8_t status_code)
{
  switch(status_code) {
    case STATUS_OK: // STATUS_OK
      //printPgmString(PSTR("ok\r\n")); break; //commented by orkun. we don't care about ok messages.
      break;
    default:
...
{
  //printPgmString(PSTR("\r\nGrbl " GRBL_VERSION " ['$' for help]\r\n")); //commented by orkun we dont need init message
}
...
void report_realtime_status()
{
  ...
  serial_write('<');
  switch (sys.state) {
  case STATE_IDLE: printPgmString(PSTR("1")); break;
  case STATE_CYCLE: printPgmString(PSTR("0")); break;
  case STATE_HOLD:
    if (!(sys.suspend & SUSPEND_JOG_CANCEL)) {
      //printPgmString(PSTR("2"));
      if (sys.suspend & SUSPEND_HOLD_COMPLETE) { serial_write('3'); } // Ready to resume
      else { serial_write('2'); } // Actively holding
      break;
  ...
  if (sys.report_wco_counter > 0) { sys.report_wco_counter--; }
    ...
    //printPgmString(PSTR("|WCO:")); //commented by orkun
    //report_util_axis_values(wco); //commented by orkun
  }
#endif
    ...
    if (sys.report_ovr_counter > 0) { sys.report_ovr_counter--; }
      ...
      //printPgmString(PSTR("|Ov:")); //these six 6 lines commented by orkun
      ...
      if (0) { //sp_state || cl_state        by orkun so its always false
